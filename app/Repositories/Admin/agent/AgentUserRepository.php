...
class AgentUserRepository
{
    ...
    public function searchUser($where, $size){
        ...
        $list = $model
            ...
            ->select(['*', 'phone as phone_hide', 'one_recommend_phone as one_recommend_phone_hide', 'two_recommend_phone as two_recommend_phone_hide', 'id as total_win_money', 'id as online_status'])
            ->paginate($size);
        return $list;
    }
...
//        DB::connection()->enableQueryLog();
        $list = $model->from('user_recharge_logs as r')
            ...
            ->with(
                [
                    'user' => function($query){
                        $query->select(['id', 'nickname', 'phone', 'total_recharge', 'cl_withdrawal', 'commission', 'one_recommend_phone', 'two_recommend_phone', 'balance']);
                    }
                ]
            )
            ->select(['r.*'])
...
//        print_r(DB::getQueryLog());die;
    ...
    public function orderInfoList($where, $size){
        $model = makeModel($where, $this->Cx_User);
        return $model
            ->select(['id', 'nickname', 'phone', 'total_recharge', 'cl_withdrawal', 'balance', 'commission', 'status', 'cl_betting', 'cl_betting_total'])
            ->with(
                [
