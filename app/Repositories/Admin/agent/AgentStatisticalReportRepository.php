...
class AgentStatisticalReportRepository
{
    ...
    public function dailyLoseRank($where, $size, $page)
    {
        $prefix = DB::getConfig('prefix');
        $offset = ($page - 1) * $size;
        $list = DB::select('select sum(gb.money) as total_betting_money, sum(gb.win_money) as total_win_money, sum(gb.service_charge) as total_service_charge, gb.user_id, u.phone from `'.$prefix.'game_betting` gb left join `'.$prefix.'users` u on gb.user_id = u.id '. $where . ' AND (total_betting_money - total_win_money) > 0' .' group by user_id order by (total_betting_money - total_win_money) desc limit '. $offset .','.$size);
