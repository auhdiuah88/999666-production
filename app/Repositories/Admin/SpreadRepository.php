...
class SpreadRepository extends BaseRepository
{
    ...
    public function findUsers($ids, $page, $size, $status)
    {
        ...
        if($status == 0){
            ...
            $all = DB::select('select sum(gb.money - gb.win_money) as cha, sum(gb.money) as total_betting_money, sum(gb.win_money) as total_win_money, sum(gb.service_charge) as total_service_charge, gb.user_id, u.phone from `'.$prefix.'game_betting` gb '. $where .' group by user_id having cha > 0');
        }
