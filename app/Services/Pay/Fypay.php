...
class Fypay extends PayStrategy
{

    protected static $url = 'https://send.dbe.one/pay';    // 支付网关

    protected static $url_cashout = 'https://send.dbe.one/applyfor'; // 提现网关
    ...
    public $company = 'fypay';   // 支付公司名  凤扬pay
    ...
    public function rechargeOrder($pay_type, $money)
    {
        ...
        \Illuminate\Support\Facades\Log::channel('mytest')->info('Fy_rechargeOrder', [$params]);

        $res = $this->requestService->postFormData(self::$url, $params);
        \Illuminate\Support\Facades\Log::channel('mytest')->info('Fy_rechargeOrder_return', [$res]);
        if ($res['status'] != 10000) {
    ...
    {
        \Illuminate\Support\Facades\Log::channel('mytest')->info('Fy_rechargeCallback',$request->post());

        if ($request->status != 10000)  {
            $this->_msg = 'Fy-recharge-交易未完成';
            return false;
        }
        // 验证签名
        $params = $request->post();
    ...
    public function withdrawalOrder(object $withdrawalRecord)
    {
        ...
        \Illuminate\Support\Facades\Log::channel('mytest')->info('Fypay_withdrawalOrder',$params);
        $res = $this->requestService->postFormData(self::$url_cashout, $params);
        \Illuminate\Support\Facades\Log::channel('mytest')->info('Fypay_withdrawalOrder',[$res]);
        if ($res['status'] != 10000) {
    ...
    {
        \Illuminate\Support\Facades\Log::channel('mytest')->info('Fypay_withdrawalCallback',$request->post());
        ...
        if ($pay_status == 0) {
            $this->_msg = 'Fypay-withdrawal-交易未完成';
            return false;
        }
        // 验证签名
        $params = $request->post();
